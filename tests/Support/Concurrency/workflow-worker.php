...
if (! $app instanceof Application) {
    throw new RuntimeException('Workflow worker could not boot the application.');
}

$kernel = $app->make(Kernel::class);
$kernel->bootstrap();
if (! $app->environment('testing') || DisposableRuntimeGuard::currentProblems() !== []) {
    throw new RuntimeException('Refusing to run Workflow worker outside a disposable testing runtime.');
}
...
if (config()->string('database.default') !== 'mysql'
    ...
    throw new RuntimeException('Workflow worker database configuration differs from its declared test connection.');
}
...
if (! is_array($arguments) || count($arguments) !== 6
    ...
    || ! in_array($arguments[5], ['return', 'reject', 'forward', 'approve', 'approval-reject', 'return-reviewer', 'return-requester', 'reopen', 'archive'], true)) {
    throw new RuntimeException('Workflow worker received invalid arguments.');
}
...
if (! is_object($connection) || ! isset($connection->id)
    ...
    throw new RuntimeException('Workflow worker could not identify its MySQL connection.');
}
...
try {
    $workflow = $app->make(NscmfWorkflowService::class);
    $lifecycle = $app->make(NscmfLifecycleService::class);
    match ($action) {
        'return' => $workflow->returnForRevision($actor, $recordId, $expectedVersion, $reason),
        'reject' => $workflow->reject($actor, $recordId, $expectedVersion, $reason),
        'forward' => $workflow->forward($actor, $recordId, $expectedVersion, null),
        'approve' => $workflow->approve($actor, $recordId, $expectedVersion, null),
        'approval-reject' => $workflow->rejectApproval($actor, $recordId, $expectedVersion, $reason),
        'return-reviewer' => $workflow->returnToReviewer($actor, $recordId, $expectedVersion, $reason),
        'return-requester' => $workflow->returnToRequester($actor, $recordId, $expectedVersion, $reason),
        'reopen' => $lifecycle->reopen($actor, $recordId, $expectedVersion, $reason, NscmfStatus::REVISION_REQUIRED),
        'archive' => $lifecycle->archive($actor, $recordId, $expectedVersion, $reason),
    };
