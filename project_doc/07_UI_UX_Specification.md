...
- **Activity** (grouped bars for four 7-day periods, one color per metric from a colorblind-validated palette, with a legend) and **Status** (one labeled row per business status with its count and a single-hue bar; decided 2026-09-24 instead of a segmented bar because Approved/Rejected green/red cannot be told apart under color-vision deficiency) render server aggregates from `12 §44.1`; every value and date range is also readable as text, without color, hover or animation;
