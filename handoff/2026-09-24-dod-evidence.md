...
| TDD RED → GREEN | Setiap perilaku baru dan setiap bug punya commit `test(...)` dengan RED bermakna sebelum commit `feat`/`fix`. Contoh sesi ini: `d435125`→`6b7cc45` (G15), `c0145e6`→`933144e` (G04 BE), `2cff504`→`430c9f4` (G04 FE), `f874894`→`c49e998` dan `77bcf69`→`bc40975` (FE-44), `2ca1755`→`b162ce4` (ekspor macet di PROCESSING). Test yang ditulis setelah perilakunya sudah ada diberi label "evidence" di pesan commit dan tidak diklaim sebagai TDD. |
